...
def createLog(foldername):
    ...
    header_format = "{:<5} {:<35} {:<25} {:<10} {:<25} {:<10} {:<15}\n"
    data_format = "{:<5} {:<35} {:<25} {:<10} {:<25} {:<10.2f} {:15.2f}\n"
    writeToFile(fobj,"-"*132+"\n")
    writeToFile(fobj,header_format.format("PID", "NAME", "USERNAME","STATUS","CREATE TIME","CPU PERCENT","MEMORY PERCENT"))
    writeToFile(fobj,"-"*132+"\n")
    for info in processDetails:

        writeToFile(fobj,data_format.format(info.get("pid"),info.get("name") or "NA",info.get("username") or "NA",info.get("status")  or "NA",info.get("create_time")  or "NA",info.get("cpu_percent") or 0.0,info.get("memory_percent") or 0.0))
        # writeToFile(fobj,"NAME : %s" % info.get("name"))
        # writeToFile(fobj,"USERNAME : %s" % info.get("username"))
        # writeToFile(fobj,"STATUS : %s" % info.get("status"))
        # writeToFile(fobj,"CREATE TIME : %s" % info.get("create_time"))
        # writeToFile(fobj,"CPU PERCENT : %s %%" % info.get("cpu_percent"))
        # writeToFile(fobj,"MEMORY PERCENT : %.2f %%" % info.get("memory_percent"))
        # writeToFile(fobj,borderFile())
    writeToFile(fobj,"-"*132)
...
def processScan():
    ...
    for proc in psutil.process_iter(attrs=["pid","name","username","status","create_time"]):
        try:
            ...
            # listProcess.append(info["pid"],info["name"],info["status"],sep="\t\t")
