...
def load_csv():
    try:
        ...
        data = pandas.read_csv("student_performance_ml_new.csv")
        print("Top 5 Records")
        print(border)
...
random_state = 10 : changes every time we run the code
random_state = 42 : Accuracy : 88.88''')
    ...
    print("Max Depth : 1 - Accuracy : 88.88")
    print("Max Depth : 3 - Accuracy : 88.88")
    print("Max Depth : None - Accuracy : 88.88")
    print("Acuuracy is same throught 3 different max depth values.")
...
def test_model(model:DecisionTreeClassifier,test_data):
    ...
    for test in test_pred:
        print("Prediction : ","Pass" if test==1 else 'Fail')
...
def main():
    ...
    if(data is not None):
        ...
        "Attendance": [65, 80, 92, 70, 88],         
        "PreviousScore": [55, 68, 85, 60, 78],    
        "AssignmentsCompleted": [3, 6, 9, 4, 8],  
        "PerfomanceIndex": [69, 90,108,78,102],    
        "SleepHours": [5, 6, 7, 5, 8]
    })
