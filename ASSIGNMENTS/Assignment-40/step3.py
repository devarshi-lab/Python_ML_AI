...
def load_csv():
    try:
        ...
        data = pandas.read_csv("student_performance_ml_3.csv")
        print("Top 5 Records")
        print(border)
...
def train_test_model(data:pandas.DataFrame):
    ...
    print(border)
    print("As we have removed sleep hours column from dataset accuracy has improved to 100%")
    print(border)
...
def feature_importance(model:DecisionTreeClassifier):
    ...
    print(border)
    print("With only two columns(Studyhours and attendance), accuracy has remain unchanged")
